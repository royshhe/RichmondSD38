...
namespace CoinDenomination
{
    class Program
    {
        ...
        {
            LinkedList<int[]> lstPossibleWay = getPossibleWay(5);
        ...
        static LinkedList<int[]> getPossibleWay(int iAmount)
        {
            ...
            for (int a = 0; a <= iMax2Euro; a++)
            {
                ...
                if (100 * iMax1Euro + 200 * a == iAmount) lstPossibleWay.AddLast(new int[8] { 0, 0, 0, 0, 0, 0, iMax1Euro, a });
                for (int b = 0; b <= iMax1Euro; b++)
                {
                    iMax50c = (int)Math.Floor((double)(iAmount - (200 * a) - (100 * b)) / 50);
                    if (50 * iMax50c + 100 * b + 200 * a == iAmount) lstPossibleWay.AddLast(new int[8] { 0, 0, 0, 0, 0, iMax50c, b, a });
                    for (int c = 0; c <= iMax50c; c++)
                    {

                        iMax20c = (int)Math.Floor((double)(iAmount - (200 * a) - (100 * b) - (50 * c)) / 20);
                        if (20 * iMax20c + 50 * c + 100 * b + 200 * a == iAmount) lstPossibleWay.AddLast(new int[8] { 0, 0, 0, 0, iMax20c, c, b, a });
                        for (int d = 0; d <= iMax20c; d++)
                        {
                            iMaxDime = (int)Math.Floor((double)(iAmount - (200 * a) - (100 * b) - (50 * c) - (20 * d)) / 10);
                            if (10*iMaxDime + 20 * d + 50 * c + 100 * b + 200 * a == iAmount) lstPossibleWay.AddLast(new int[8] { 0, 0, 0, iMaxDime, d, c, b, a });
                            for (int e = 0; e <= iMaxDime; e++)
                            {

                                iMaxNickel = (int)Math.Floor((double)(iAmount - (200 * a) - (100 * b) - (50 * c) - (20 * d) - (10 * e)) / 5);

                                if (5* iMaxNickel  + 10 * e + 20 * d + 50 * c + 100 * b + 200 * a == iAmount) 
                                ...
                                for (int f = 0; f <= iMaxNickel; f++)
                                {
                                    ...
                                    if (iMax2c * 2 + f * 5 + 10 * e + 20 * d + 50 * c + 100 * b + 200 * a == iAmount) lstPossibleWay.AddLast(new int[8] { 0, iMax2c, f, e, d, c, b, a });
